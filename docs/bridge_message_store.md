...
`ChannelBridgeManager` は Discord 上で処理した表示名・アイコン URL・DiceBear 失敗フラグ・送信先メッセージ ID・添付ファイル情報を PostgreSQL の `bridge_messages` テーブルに保存します。編集同期やリアクション処理ではこのメタデータを参照し直すため、定期的なクリーンアップを推奨します。

## 古いレコードを定期削除する

24 時間を超えて編集される可能性が低いため、cron などから次のスクリプトを実行して古いレコードを削除してください。`DATABASE_URL` を環境変数で渡すと、Bot 起動時と同じ接続先にアクセスできます。
...
再同期後は、対象メッセージを再送信または編集することで新しいルートに沿ったメタデータが作成されます。必要なら `docs/guide/postgresql_setup.md` を参照し、Postgres 側のテーブル定義も確認してください。
